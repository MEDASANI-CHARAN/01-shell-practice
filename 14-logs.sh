...
mkdir -p $LOGS_FOLDER
echo "Script started ececuting at: $(date)" &>>$LOG_FILE
...
then
    ...
    echo -e "MySQL is already installed... $Y SKIPPING $N" &>>$LOG_FILE
fi
...
then
    ...
    echo -e "nginx is already installed... $Y SKIPPING $N" &>>$LOG_FILE
fi